...
import { HSLColor, RGBColor, blendColors, hsl, hslToRgb, rgbToHsl, rgba } from "@/app/utils/colors";
import { CELESTIAL_ANGLES, MOUNTAIN_MATERIAL } from "../consts";
...
export default function drawSineWaveMountain(
  ...
  // const adjustedColor = rgbToHsl(blendColors(hslToRgb(MOUNTAIN_MATERIAL), sunColor));
  const baseColoredLightTransparency = 0.1;
  ...
  const sunMaxLightness = 50 * percentTowardForeground;
  const moonMaxLightness = 30 * percentTowardForeground;
